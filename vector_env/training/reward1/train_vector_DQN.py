...
class DetailedMetricsCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        #    Falls diskrete Actions, ist das typischerweise ein np.array([...])
        if "actions" in self.locals:
            actions = self.locals["actions"]
            self.episode_actions.extend(actions)

        # 5) (Optional) Loss
        #    Nur wenn du selbst "loss" in self.locals platzierst (z.B. in einer custom DQN train-Methode)
    ...
    def _on_rollout_end(self):
        ...
        Bei DQN kann das mitten in einer Episode sein, daher meist kein spezielles Logging nötig.
        """
        pass
...
if __name__ == "__main__":
    ...
    eval_callback = EvalCallback(
        eval_env=eval_env,
        best_model_save_path="vector_env/models/DQN/training6",
        log_path="vector_env/logs/DQN/training6",
        eval_freq=10000,
        n_eval_episodes=10,
        deterministic=True,
        render=False,
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=50000,
        save_path="vector_env/models/checkpoints/DQN/training6",
